...
template sudoku() {
    ...
    //Declare the RangeProof components for puzzle and solution
    component puzzleRangeProof[9][9]; 
    component solutionRangeProof[9][9]; 

    //Initialize the above RangeProof components 
    //RangeProof checks if a number is within a certain range
    //RangeProof templates are passed the parameter 4 since upper bound 9 has 4 bits (1001)
    for(var i = 0; i < 9; i++){
        for (var j=0; j<9; j++) { 
            puzzleRangeProof[i][j] = RangeProof(4);
            solutionRangeProof[i][j] = RangeProof(4);
        }
    }
    
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            puzzleRangeProof[i][j].in <== puzzle[i][j]; //set number to check in puzzleRangeProof
            puzzleRangeProof[i][j].range[0] <== 0; //set lower bound in puzzleRangeProof
            puzzleRangeProof[i][j].range[1] <== 9; //set higher bound in puzzleRangeProof
            puzzleRangeProof[i][j].out === 1; // check if its 1(true), throws an error if false
            solutionRangeProof[i][j].in <== solution[i][j]; //set number to check in solutionRangeProof
            solutionRangeProof[i][j].range[0] <== 0; //set lower bound in solutionRangeProof
            solutionRangeProof[i][j].range[1] <== 9; //set higher bound in solutionRangeProof
            solutionRangeProof[i][j].out === 1; // check if its 1(true), throws an error if false
